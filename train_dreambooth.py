    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                avg_timestep = np.mean(timesteps_cpu)  # 计算batch的平均timestep  
                time_loss_log.append((avg_timestep, loss_cpu))
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        #!!
                        # os.makedirs("./wzh_save", exist_ok=True)
                        # np.save('./wzh_save/wzh_loss_adv.npy', loss_all_list)
                        np.save(
                            os.path.join(args.output_dir, "wzh_loss_adv.npy"),
                            np.asarray(loss_all_list, dtype=np.float32)
                        )
                        # arr = np.asarray(time_loss_log, dtype=np.float64)
                        # np.save(
                        #     os.path.join(args.output_dir, "neo_loss_adv.npy"),
                        #     arr
                        # )
                        if len(time_loss_log) > 0:  
                            timesteps = [pair[0] for pair in time_loss_log]  
                            losses = [pair[1] for pair in time_loss_log]  
      
                            # 结构化数组  
                            arr = np.array(list(zip(timesteps, losses)),   
                                            dtype=[('timestep', 'f8'), ('loss', 'f8')])  
                            np.save(  
                                os.path.join(args.output_dir, "neo_loss_adv.npy"),  
                                arr  
                            )
